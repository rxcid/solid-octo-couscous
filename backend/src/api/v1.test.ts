...
import type { Relationships, Resolution, SiblingGroup, TrackDetail, TrackSummary } from "./schemas.js";
...
describe("GET /v1/tracks/:id/relationships", () => {
  it("covers the whole song, one relationship per related song, never candidates", async () => {
    ...
    assert.deepEqual(
      body.derivatives.items.map((r) => [r.source.title, r.type, r.destination.title]),
      [
  ...
  it("hides relationships between recordings of the same song", async () => {
    ...
    assert.equal(echo!.type, "interpolated");
  ...
  it("keeps different relationship types apart and includes non-song sources", async () => {
    ...
    assert.deepEqual(
      echo.sources.items.map((r) => [r.type, r.source.title]),
...
describe("the service", () => {
  it("publishes an OpenAPI document for every route", async () => {
    const spec = await get<{ openapi: string; paths: Record<string, unknown> }>("/openapi.json");
    assert.equal(spec.openapi, "3.1.0");
    assert.deepEqual(Object.keys(spec.paths).sort(), [
      "/v1/search",
      "/v1/tracks/resolve",
      "/v1/tracks/{id}",
      "/v1/tracks/{id}/relationships",
      "/v1/tracks/{id}/siblings",
    ]);
  });
